...
//Remove this ipp reference
#include <answer/archive/impl/basic_ws_xml_grammar.hpp>


namespace answer{
namespace archive{

typedef answer::archive::basic_ws_xml_grammar<char> xml_grammar;
...
// 	xmlpp::DomParser _parser;
	...
	struct load_enum_type {
		...
		static void invoke(Archive &ar, T &t){
			std::string text;
			bool result = ar.parseString(text);
			if (!result){
...
// 	}
	...
	bool parseString(std::string& str){
		return _gimpl->parse_string(_is, str);
	}
...
// 	}
	...
	void load(std::string & str){
		bool result = _gimpl->parse_string(_is, str);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
	}
	...
	void load (bool & t){
		std::string text;
		bool result = _gimpl->parse_string(_is, text);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
...
// 					return *this;
		...
		if (!isNewEntry){
			bool result = _gimpl->parse_start_tag(_is);
			if (!result){
				throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
			}
		}
		...
		if (_gimpl->rv.object_name != m_currentName) {
			std::cerr << "Expected element [" << m_currentName  << "] got [" << _gimpl->rv.object_name << ']' << std::endl; 
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
		(* this) >> t.value();
		
		bool result = _gimpl->parse_end_tag(_is);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
		return * this;
	...
	template<class T>
	ws_xml_iarchive & operator>>(const boost::serialization::nvp< boost::optional<T> > & t){
		bool result = _gimpl->parse_start_tag(_is);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
		if (_gimpl->rv.object_name == t.name()) { // This optional is defined
			T aux;
			(* this) >> aux;
			t.value() = aux;
		}
		result = _gimpl->parse_end_tag(_is);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
		return * this;
	...
	ws_xml_iarchive(std::istream & is):
		_is(is),
		_gimpl(new xml_grammar())
	{
		_gimpl->init(_is);

		//Consume first tag
		bool result = _gimpl->parse_start_tag(is);
		if (!result){
			throw boost::archive::archive_exception(boost::archive::archive_exception::input_stream_error);
		}
	}
