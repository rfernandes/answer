...
/////////1/////////2/////////3/////////4/////////5/////////6/////////7/////////8
// template code for basic_ws_xml_grammar of both wchar_t and char types
...
bool basic_ws_xml_grammar<CharType>::my_parse(
    BOOST_DEDUCED_TYPENAME basic_ws_xml_grammar<CharType>::IStream & is,
    const rule_t & rule_,
    CharType delimiter
) const {
    if(is.fail()){
    ...
    CharType val;
    do{
        BOOST_DEDUCED_TYPENAME basic_ws_xml_grammar<CharType>::IStream::int_type
            result = is.get();
        if(is.fail())
            return false;
...
template<class CharType>
bool basic_ws_xml_grammar<CharType>::parse_start_tag(
    BOOST_DEDUCED_TYPENAME basic_ws_xml_grammar<CharType>::IStream & is
){
...
template<class CharType>
bool basic_ws_xml_grammar<CharType>::parse_end_tag(IStream & is) const {
    return my_parse(is, ETag);
}

template<class CharType>
bool basic_ws_xml_grammar<CharType>::parse_string(IStream & is, StringType & s){
...
template<class CharType>
basic_ws_xml_grammar<CharType>::basic_ws_xml_grammar(){
    init_chset();
...
template<class CharType>
void basic_ws_xml_grammar<CharType>::init(IStream & is){
    init_chset();
...
template<class CharType>
void basic_ws_xml_grammar<CharType>::windup(IStream & is){
    if(is.fail())
        return;
